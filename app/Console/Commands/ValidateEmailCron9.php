...
class ValidateEmailCron9 extends Command
{
     ...
     * @var string
     */
    protected $signature = 'validate:emailcron9';
    ...
    public function handle()
    {
        ...
        if ($emails->count() > 0) {
            ...
            $result = Emails::whereIn('matched_contact_id', $plucked_email_array)->update(['status' => 'cron9']);
                                ...
                                if($v_response['email_status'] == "invalid"){
                                    ...
                                    if(strlen($v_response['response']) > 0){
                                        ...
                                        if(isset($response_api_array['error']) && $response_api_array['error']['code'] == 104){
                                            Emails::where('status', 'cron9')->orWhere('email',$email)->update(['status' => 'success']);
