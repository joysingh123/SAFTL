...
class ValidateEmailCron3 extends Command {
     ...
     * @var string
     */
    protected $signature = 'validate:emailcron3';
    ...
    public function handle() {
        ...
        if ($emails->count() > 0) {
            ...
            $result = Emails::whereIn('matched_contact_id', $plucked_email_array)->update(['status' => 'cron3']);
                            ...
                            if ($v_response['email_status'] == 'valid' || $v_response['email_status'] == 'catch all') {
                                ...
                                if($v_response['email_status'] == "invalid"){
                                    ...
                                    if(isset($response_api_array['error']) && $response_api_array['error']['code'] == 104){
                                        Emails::where('status', 'cron3')->orWhere('email',$email)->update(['status' => 'success']);
